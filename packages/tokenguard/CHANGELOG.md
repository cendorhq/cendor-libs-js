...
- 44b2bfb: `StalePriceTableWarning` — warned **once per process** when a USD budget estimates from a price table
  ...
  A USD cap is only as right as the rates behind it, and the direction matters: after a price _cut_ a
  stale table over-estimates and the cap binds early, which is conservative; after a price _rise_ it
...
- 0a08e89: Accept a real OpenTelemetry `Meter` where meter injection is documented.
