...
| `instrument()` structured output (`responses.parse`) | ✅ (≥ 1.14.1) | ✅ (≥ 0.16.1) | it issues its own request rather than delegating to `create`, so a structured-output call used to emit nothing at all. `chat.completions.parse` remains unwrapped in both |
