...
| guardrails local embedder (G2) | ✅ `embeddings.local_embedder` | 🚧 planned | the zero-config `embed` for the semantic checks (model2vec static embeddings, numpy-only) is **Python only** — no maintained model2vec JS port. In TS pass a BYO `embed` (transformers.js / a hosted endpoint). |
