...
| guardrails `taskAdherence` (A3) | ✅ `task_adherence` | ✅ helper / 🚧 SDK wiring | BYO-judge alignment check for the `tool_call` stage (does the proposed call match the user's intent?), via `judge.taskAdherence` + the new optional `Context.instruction`. The `@cendor/guardrails` helper is at parity; the **`@cendor/sdk`** auto-threading of the instruction is the deferred tail (set `ctx.instruction` yourself until it lands). |
